...
public class GAS_ProportionalGI2 implements GAS_interface {
	public Pattern pattern;
	public float spread_factor;
	public boolean labeled;
	private Partition p;
	
	public GAS_ProportionalGI2(Partition p)
	{
	...
	public GAS_ProportionalGI2(String patternFilePath, boolean labeled, float proportion, Partition p)
	{
		pattern = new Pattern(patternFilePath, labeled);
		if(proportion >= 0.0f && proportion <= 1.0f)
			spread_factor = proportion;
		else
		{
			spread_factor = 0.01f;
			System.out.println("Paremeter spread factor is not between 0 and 1, default value 0.01 is used.");
		}
...
/*		VertexDataGI data;
	...
	public GatheredData gather(Vertex myself, Vertex neighbor, Edge e) {
		...
		//random control
		int removed = m_in_v.messages.size() - (int) Math.ceil(spread_factor*m_in_v.messages.size());
		long seed = System.nanoTime();
		Collections.shuffle(m_in_v.messages, new Random(seed));
		m_in_v.messages.subList(0, removed).clear();
